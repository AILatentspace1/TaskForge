...
2. If `<runtime_dir>\LOCK` exists and mtime is less than 30 minutes ago, exit silently because heartbeat may be mid-run.
...
2. Append one JSONL event to `<runtime_dir>\log\<YYYY-MM-DD>.jsonl`: `{ts, action:"retro", summary, file:"daily/<date>.md"}`.
