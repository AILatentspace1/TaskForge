...
2. If `<runtime_dir>\LOCK` exists:
   ...
   - Else treat it as stale and overwrite it with `{pid:<yours>, started_at:<ISO>, automation:"team-heartbeat"}`.
...
2. Append one JSONL event to `<runtime_dir>\log\<YYYY-MM-DD>.jsonl`: `{ts, run_id, action, task_id?, state?, summary}`.
