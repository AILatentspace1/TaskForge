...
5. Read `<runtime_dir>/scratch/risk-scan.json` if present for audit risk context.
6. Read newest 5 `<runtime_dir>/log/*.jsonl` entries for audit trail continuity.
...
For each state change detected this run, record a structured audit entry:
- Task ID, from → to state, timestamp, trigger (retro/heartbeat/manual), run ID
- Include AI decision rationale when applicable (e.g., why a task was parked or blocked)
- Flag any unexpected state transitions for human review

Summarize audit findings:
- Total state changes this run: <n>
- AI-initiated changes: <n>
- Unexpected transitions: <n>

## Recurring pattern detection

Analyze historical log data (`<runtime_dir>/log/*.jsonl`) for patterns:
- Tasks that cycle between states (e.g., in-progress → review-ready → in-progress more than 2 times)
- Repeated CI failures on the same task
- Tasks consistently blocked by the same dependency
- Planner candidates frequently skipped for the same reason

For each detected pattern:
- Describe the pattern and frequency
- Recommend a structural fix (e.g., split task, update dependency, refine DoD)
- If actionable, write the recommendation as a signal to `<runtime_dir>/policies/signals.md`
```
